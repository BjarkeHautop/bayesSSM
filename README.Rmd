...
parameters. The function `log_likelihood_fn` should be a function that
...
Note, that `init_fn` must take an argument `particles` and return a vector
of initial values for the latent state variables. `transition_fn` must take
arguments `particles`, and `log_likelihood_fn` must take arguments `y` and
...
100 pilot samples and a burn-in of 10. 
...
- Initial distribution $\mu(X_1)$ that describes the distribution of the 
  latent state variables at time $t = 1$. Defined by `init_fn` in `pmmh`.
...
particles and the proposal distribution for the parameters. The control of the 
tuning can be modified by the argument `tune_control`.
