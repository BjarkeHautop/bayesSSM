...
bayesSSM is an R package offering a suite of tools for performing Bayesian 
inference in state-space models (SSMs). It implements 
...
We can use `pmmh` to perform Bayesian inference on this model. We need to define
...
hidden (latent) state variables. The typical SSM is structured as a 
...
particles and the proposal distribution for the parameters, which can be modifed
by `default_tune_control`.
