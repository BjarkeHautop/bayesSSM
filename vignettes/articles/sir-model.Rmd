...
vignette we only use a small number of iterations (1000) and 2 chains.
In practice, these should be higher.

```{r}
result <- bayesSSM::pmmh(
  y = observations,
  m = 1000,
  init_fn_ssm = init_fn_epidemic,
  transition_fn_ssm = transition_fn_gillespie,
  log_likelihood_fn_ssm = log_likelihood_fn_epidemic,
  log_priors = log_priors,
  init_params = c(lambda = 0.5, gamma = 0.5),
  burn_in = 200,
  num_chains = 2,
  param_transform = list(lambda = "log", gamma = "log"),
  tune_control = default_tune_control(pilot_proposal_sd = 0.1),
  verbose = TRUE,
  seed = 1405,
)
```
