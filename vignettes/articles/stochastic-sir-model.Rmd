...
matrix_exp(t) = e^{Qt},
$$
where $generator$ is the generator matrix of the Markov process, which contains
...
state and time point. For example, if we have a population of size $pop_size=5$
with state $(S(t), I(t)) = (4, 1)$ at time $t$, we can compute the transition
...
$$
which for $pop_size=100$ is already over $5000$ states. Thus, this whole
approach is only feasible for very small populations. 
