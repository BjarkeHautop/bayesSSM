...
```{r}
ggplot() +
  geom_line(aes(x = 1:t_val, y = x), color = "blue", linewidth = 1) +  # Latent state
  geom_point(aes(x = 1:t_val, y = y), color = "red", size = 2) +  # Observed values
  labs(title = "Simulated Data: Latent State and Observations", 
       x = "Time", 
       y = "Value",
...
We fit 2 chains with $m=2000$ iterations for each, with a burn_in of $500$.
In practice you should run more iterations and chains. To improve sampling
we specify that metropolis hastings should specify $\sigma_x$ and $\sigma_y$ on
the $\log$-scale. 
...
ggplot2::ggplot() +
  ...
  labs(title = "Density plot of phi chains", 
       x = "Value", 
       y = "Density",
