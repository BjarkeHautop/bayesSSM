...
vignette: >
  %\VignetteIndexEntry{Detailed Overview of bayesSSM Package}
...
In this vignette, we will provide a detailed overview of the `bayesSSM` package. 
...
distribution is
$$
p(x_{0:T}, \theta \mid y_{1:T}) \propto \pi(\theta) \, p(x_{0:T} \mid \theta) 
...
Some naive MCMC approaches would be to sample $(x_{0:T}, \theta)$ jointly, but 
this is often infeasible due to the high dimensionality of the latent states. 
...
see also the sir-model article in the `bayesSSM` package). First, we
will generate some synthetic data from the model.
...
# Conclusion
In this vignette, we have provided a detailed overview of the `bayesSSM` package 
