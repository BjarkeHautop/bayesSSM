...
.run_pilot_chain_new <- function(y, pilot_m, pilot_n, pilot_reps, init_fn_ssm,
  ...
  # Check if init_params are valid
  ...
  pf_result <- particle_filter(y, pilot_n,
                               init_fn = init_fn_ssm,
                               transition_fn = transition_fn_ssm,
                               log_likelihood_fn = log_likelihood_fn_ssm,
                               algorithm = algorithm,
  ...
  # To do implement jacobian for transformation
  ...
  for (m in 2:pilot_m) {
    ...
    log_jacobian_ratio <- prop$log_jacobian
    ...
    pf_prop <- particle_filter(y, pilot_n,
                               init_fn = init_fn_ssm,
                               transition_fn = transition_fn_ssm,
                               log_likelihood_fn = log_likelihood_fn_ssm,
                               algorithm = algorithm,
  ...
  pilot_result <- .pilot_run(y, pilot_n, pilot_reps, init_fn_ssm,
                             transition_fn_ssm, log_likelihood_fn_ssm,
                             algorithm = c("SISAR", "SISR", "SIS"),
                             resample_fn = c("stratified", "systematic",
                                             "multinomial"),
